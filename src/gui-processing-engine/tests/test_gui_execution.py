...
@mock.patch("gui_server.subprocess.Popen")
@mock.patch("gui_server.subprocess.run")
@mock.patch("builtins.open", mock.mock_open())
@mock.patch("gui_server.os.makedirs")
@mock.patch("gui_server.os.chown", create=True)
def test_python_tk_execution(mock_chown, mock_makedirs, mock_run, mock_popen):
    """
    ...
    payload = {
        "language": "gui-python",
        "source_code": "import tkinter as tk\nroot = tk.Tk()\nroot.title('Test')\n"
    }
...
@mock.patch("gui_server.os.makedirs")
@mock.patch("gui_server.os.chown", create=True)
def test_java_gui_compilation_and_execution(mock_chown, mock_makedirs, mock_run, mock_popen):
    """
...
@mock.patch("gui_server.os.makedirs")
@mock.patch("gui_server.os.chown", create=True)
def test_hot_reload_terminates_previous_process(mock_chown, mock_makedirs, mock_run, mock_popen):
    """
    ...
    # Execution 1
    client.post("/api/v1/execute", json=payload)
